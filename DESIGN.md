...
Two relationships matter, and they are not the same. The Enscribe shorthand and eHTML are a **lossless round-trip**: they are one document in two notations — the shorthand is simply a faster way to type the canonical form, and any eHTML document can be written back as shorthand. Export sits outside that loop: eHTML **exports** to several interchange formats — JATS, TEI, EPUB, and Scholarly HTML — as clean, one-way operations, with import from those corpora into eHTML supported but acknowledged-lossy (a conversion that applies necessary simplifications, not a faithful reconstruction). JATS is the most mature scholarly-interchange schema and the best-developed of these exports today, but it is one target among several equally positioned, not a privileged anchor: eHTML is a primary rich-document HTML vocabulary in its own right, useful as HTML before any export runs (this extends the "reference standards: guide, don't gate" decision in `notes/decisions.md`, which positions JATS alongside TEI / Scholarly HTML as a reference standard rather than a first-class gate).
...
**JATS as one export target.** Enscribe eHTML compiles to JATS XML via the CLI's `enscribeToJats` exporter. This makes enscribe documents submittable to journals and ingestable by the scholarly publishing ecosystem (PubMed, CrossRef, archival systems) without requiring Pandoc as a runtime dependency or hand-conversion. It is the best-developed of enscribe's exports today, and it sits alongside the other interchange targets (TEI, EPUB, Scholarly HTML) rather than above them. The pitch is not "academic markdown for the web" but "academic markdown for the web that can submit to journals."
...
The relationship is instructive, and its *direction* matters. JATS is a vocabulary enscribe *consults* when growing eHTML, and a format enscribe *exports* to — but it does not *shape* eHTML. **eHTML is HTML-shaped, not JATS-shaped.** An enscribe document is HTML internally, authored through the registers; its HTML render is a (near-)identity projection rather than a translation, and any interchange export (JATS, TEI, EPUB, Scholarly HTML) is the translation — a feature, not the mission. The mission is rich documents that render as HTML; the exports are features reached *from* that base, so the translation cost belongs on the export, not the render. Where JATS has 200-plus XML elements and no display target, eHTML is a manageable set of HTML elements that render directly in a browser and translate cleanly to JATS — and to the other interchange targets — on export. (This reframes an earlier "eHTML is a projection of JATS" framing; the migration to a fully HTML-shaped vocabulary is tracked in [#147](https://github.com/enscribejs/enscribe/issues/147) and is described honestly in "The vocabulary-boundary principle" below.)
...
A companion of that rule governs the **derived-display layer** — the generated numbers, labels, ToC chrome, diagnostic markers, and sidenote clones the compiler synthesizes for presentation (not authored, not archival: JATS reconstructs each `<label>`/`<xref>` from the node structure, never from these). Ariel's ruling: **everything eHTML emits is a semantic element, not a class on a meaningless span.** So a derived-display artifact is a named custom element — `<section-number>`, `<figure-label>`, `<equation-number>`, `<toc-num>`, `<parse-error>`, `<sidenote>`, … — whose element *name* is the semantic, rather than a `<span class="section-number">`. This is distinct from the semantic-gap rule above (which is about *vocabulary* HTML lacks a native element for, e.g. a citation): a derived-display element names a *rendering* of a node property, and its being a real element is also what a future in-browser derivation engine finds and populates. (Two output spans deliberately remain: KaTeX's own math markup, and the formatter's transient inline-shim.)

**Migration state — honest.** This is the *decided direction*, not a finished state. As of v0.4.0, **lists are the inaugural case**: `<list>` / `<li>` author the construct and lower to plain `<ul>` / `<ol>` / `<li>`. **Figures are the second migrated group** ([#147](https://github.com/enscribejs/enscribe/issues/147)): the canonical eHTML element is HTML-native `<figure>` / `<figcaption>` (the figure handler already emits it), with `<fig>` retained as the JATS export name and an authoring alias. The rest of the vocabulary — sections, the semantic-only elements — still renders as custom eHTML elements today and migrates element-group by element-group under the tracking epic, [#147](https://github.com/enscribejs/enscribe/issues/147), each step gated by two byte-identical invariants (the HTML render and the JATS export must not change). The model is the target the vocabulary is moving toward; it is not yet fully realized.
...
**The lossy lift.** Converts the Enscribe shorthand's markdown idioms *up* to eHTML canonical. **Lift-only** (one-way; eHTML lowers to canonical-named-form, not to bare markdown), **lossy** (a markdown idiom may have multiple canonical representations, and one is chosen), a small set of transforms. Each rule is a per-construct rewrite — an mdast `heading` and a canonical section node are different shapes, not the same shape with a different tagname.
...
The exception is one-directional: lifted into the rendered output, not part of the round-trip cipher. Lowering eHTML → canonical that encounters an `<h4>` in the input is a separate policy decision deferred to the lowering work.
...
eHTML output maps cleanly to either DTD without per-document
restructuring.
...
JATS export is *in* scope (see "JATS as reference and export target" above). Render mode — the lossy lowering of eHTML to plain HTML headings — is also *in* scope: it is the third rung of the display ladder, not a discarded alternative to it. The project's contribution is the specification (eHTML, the canonical semantic form), the Enscribe shorthand, the glue plugins that connect them to the existing ecosystem, the display targets that render eHTML for different consumers, and the interchange exports to scholarly-publishing formats (JATS, TEI, EPUB, Scholarly HTML).
...
|---|---|---|---|---|---|---|---|---|---|---|---|
| **Enscribe** | ✓ | ✓ (CSL) | ✓ (scoped) | ✓ (KaTeX) | ✓ | ✓ | ✓ | ✓ | ✓ (eHTML) | ✓ (sigils) | — (in development) |
...
- **Not a one-vendor ecosystem.** Quarto is a single-vendor project (Posit). Enscribe is intended to be community-developable; the eHTML vocabulary is a stable target that multiple implementations could share.
...
- **JATS** — Lossless export (done). Import (planned). Round-trip JATS → Enscribe → JATS should preserve content faithfully, modulo some DSL handling. JATS is the most mature of enscribe's interchange targets, sitting alongside TEI, EPUB, and Scholarly HTML rather than above them.
