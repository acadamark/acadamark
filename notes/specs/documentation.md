...
7. **Failure behavior is demonstrated, not just named (#395/D1).** The authoring guide's
   ...
   library (rendered in the chapter body against the chapter's own small `<library>`, whose
   resolved cite also demonstrates the auto-placed References list). Resolution failure is
   part of the authored surface; the docs show it the way they show every other rendered
   result.
