...
`packages/ehtml/SPEC.md` and the individual entries in
`packages/ehtml/elements/`. For the recursive-content plugin
...
imports `VOCABULARY` from the `ehtml` package and wraps it in a
...
`packages/ehtml/src/data.js` and is committed to the repo. It
is regenerated by `packages/ehtml/build/generate-data-module.js`
(run via `npm run build` in `ehtml`) from the source `.md` files
under `packages/ehtml/elements/`. A `pretest` staleness guard
in `ehtml` regenerates into a temporary path and diffs against
...
1. Create `packages/ehtml/elements/new-element.md` with YAML
...
- `packages/ehtml/SPEC.md` — vocabulary element specification.
