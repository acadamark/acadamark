...
- **`RQ-BOOK-M8`** (separate-pages publish) — a book built with `enscribe build`
  ...
  per-chapter URL (the title slug — number-free, so a chapter URL is stable whether or not section
  numbering is on; an `index.html` lands the set). Each
  ...
  (`counting-elephants.html#fig:transect`); an in-chapter reference stays an
