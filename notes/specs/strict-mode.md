...
- **The reparse needs the WHOLE document's source, so it applies to single-source documents only — for now (#451).** The re-parse reads the source from the run's VFile (`file.value`). A single-source document — a plain article, or a single-file book/website whose chapters/pages are authored inline — carries its whole source there, so the register applies. An **assembled** document does not: a multi-file master (`<chapter src>` / `<section src>` / `<include>`) is stitched together at the *tree* level from many files, and a website/book master builds each page from its own source, so there is no one source string that reproduces the assembled tree. The reparse detects this (an empty source, or a reparse that still carries top-level `<… src>` / `<include>` entries) and **skips the swap**: the fully-parsed tree stands (all registers on, nothing dropped) and the document emits a `strict-mode:assembled-unsupported` diagnostic rather than silently ignoring the setting. Before the guard, the assemble paths built the VFile with no source and the reparse of the empty string wiped the whole document to an empty `<article>` (the #451 data loss). Applying the register to a multi-file body needs registers-off **assembly** (each child reparsed with the off-processor before stitching) — a follow-on to #451, not built here.
