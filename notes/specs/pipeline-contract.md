...
| 12.5 | `enscribeQuietSuppression` (anon inline fn) | 3 | `src/interpreter/index.js` (inline) | #281/#450 — a `<config quiet />` document clears its own `file.messages` stream (emission-only; tree/HTML byte-identical). This is the TRANSFORM tail; the compiler emits more (unknown-tag/handler-error #412, sidenote block-content), so the compiler clears again at its own tail (`compileToHtml` → `suppressMessagesIfQuiet`) — the transform tail alone does NOT catch every producer | after `enscribeConfigDiscovery`; the compiler-tail clear runs after every producer |
