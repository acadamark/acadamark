...
- **Markdown emphasis maps to the *visual* tags.** `*x*` normalizes to `<i>` and `**x**` to `<b>` — not the semantic `<em>` / `<strong>`. The semantic tags are reached only by writing them explicitly. Headings of depth 1–3 lift to the section ladder; depths 4–6 pass through as literal `<h4>`–`<h6>` (the named exception). Explicit markdown links `[text](url)` are **not** an idiom — they render as their literal source (a bare URL / email autolink still lifts to `<a>`). That literal is emitted as inline text **spliced in place** at the normalization gate (`liftLink` returns the `[`, the link's own inline children, and the `](url)` as sibling nodes via `walkNormalize`'s 1-to-N form) — **no `<span>` wrapper**. An earlier implementation wrapped the literal in a `<span>`, which reached tag-dispatch with no `span` handler (span was retired from the vocabulary, #334) and emitted a spurious `unknown tag <span>` warning; splicing bare text keeps the same rendered output without minting an unhandled tag. The bare markdown image `![alt](url)` is the same non-idiom, emitted as a literal text node.
- **GFM footnotes and reference-link definitions are not idioms.** GFM footnote syntax — the reference `text[^1]` and the definition line `[^1]: body` — is **not** an enscribe idiom (#407). Enscribe has a superior native footnote: the `<note>` tag, and its sigil `<^ …>` (#416). Markdown footnotes render as their **literal source** — `[^1]` and `[^1]: body` show as the characters typed — the same honest degrade every unsupported markdown form gets, and, crucially, **no authored content is lost** (the earlier half-processing rendered a live-looking marker while dropping the footnote body — a content-loss trap). Reference-style link **definition lines** (`[label]: url`) render literally too, by the same principle: the reference `[text][label]` was already literal (markdown links are not an idiom, above), and its definition line must match rather than silently vanish. All three forms are spliced in place as literal text at the normalization gate, exactly as the markdown-link literal is — no wrapper element, no lost content. (There is no canonical-node row for them because they normalize to *text*, not to an enscribe element.)
- **A heading is a section, never a title.** The rows above already say `#` / `##` / `###` lift to `section` / `sub-section` / `sub-sub-section`; the corollary is that no heading is ever the document *title*. `#` nests normally, and a lone `#` wrapping a whole document is a valid one-section structure, not a bug. The document title comes only from `<meta>` (the `<title>` inside it) — with no `<meta>` title the document has no title (blank; a valid authoring choice, not an error). See `pipeline.md` §"Phase 2 — Structural transformation".
- **Not every markdown construct is normalized.** Fenced code blocks (` ``` `) currently render through the native remark→hast path; they are a candidate idiom not yet on the normalization registry, so they have no canonical-node row above. (Bulleted / ordered lists *were* in this category; as of #137 they are a first-class construct — the `-` / `*` idiom is one item register of `<list>` / `<li>`, alongside the `<->` / `<*>` sigils and the canonical marker. See `lists.md`.) Per the registry framing below, whether a construct is normalized *today* is a `STATUS.md` / backlog question, not this document's.
