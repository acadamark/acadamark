# enscribe-core: the inward-pointing shared package

This is the architecture-decision record (ADR) for the `enscribe-core`
...
Enscribe's package set is organized around a small `enscribe-core` package
that all output-producing packages depend on. The dependency graph points
**inward**: `enscribe-core` depends on nothing internal; `remark-enscribe`,
`enscribe-interpreter`, the forthcoming JATS-export package, and any future
output generator all depend on it. `layer1-vocabulary` is a separate leaf
that depends on nothing internal and is consumed (alongside `enscribe-core`)
by the output-producing packages.

The arc resolved DRY-audit Bin A.1 (the canonical `enscribeTag` shape was
...
```
                ...
                └──────────────────────────────────────┘
                  ...
                  │              │              │
       ┌─────────────────┐   ┌──────────────────┐   ┌──────────────────────────┐
       │ remark-enscribe│   │ enscribe-       │   │ enscribe-jats-export    │
...
- `remark-enscribe` → `enscribe-core` (uses `dsl-registry`,
  `tagname-sigil-map`, tag builders, error-node builders).
- `enscribe-interpreter` → `enscribe-core` (uses everything in core);
  `enscribe-interpreter` → `remark-enscribe` (legitimate parser→interpreter
  stage edge: imports `recursive-content` and the parser's unified-plugin
  glue);
  `enscribe-interpreter` → `layer1-vocabulary` (imports the generated
  `VOCABULARY` data module).
- `enscribe-jats-export` (future) → `enscribe-core` + `layer1-vocabulary`.
  It will NOT depend on `enscribe-interpreter` — JATS export consumes Layer 1
  hast/mdast, not the interpreter's HTML.
- `layer1-vocabulary` → nothing internal (leaf).
- `enscribe-core` → nothing internal (leaf).

No cycles. No sideways edges between consumers. Every shared global-design
fact lives in `enscribe-core` (the home for facts that belong to the project,
...
|---|---|---|
| `src/tag.js` | The canonical `enscribeTag` node shape; builders `makeTag`, `makeOpaqueTag`, `makeInternalMarker`; the `isEnscribeTag` predicate | G1 |
| `src/error-nodes.js` | The canonical `enscribeParseError` / `enscribeTagError` shapes; builders `makeParseError`, `makeTagError` | G7 |
...
| `src/file-data-keys.js` | String constants for `file.data.*` plugin-bus keys (`ENSCRIBE_CONFIG`, `ENSCRIBE_REGISTRY`, `ENSCRIBE_CITATIONS`, `ENSCRIBE_NOTES_PENDING`, `ENSCRIBE_NUMBERING_PENDING`) | G12 |
...
The Peggy grammar compiler (`remark-enscribe/build/compile-grammar.js`) and
...
The HTML attribute mapper (`enscribe-interpreter/src/lib/build-properties.js`)
...
The iteration shape lifted to `enscribe-core` as
`mapAttributes(node, vocab, target, emit)`
(`packages/enscribe-core/src/map-attributes.js`). The lift waited for a
second output-target consumer; JATS export (Phase 5) is that second
consumer. The HTML side (`enscribe-interpreter/src/lib/html-emit.js`)
and the JATS side (`enscribe-jats-export/src/lib/jats-emit.js`) each
...
═══════════════════════════════════════════════════════════════════════════
  • remark-enscribe/build/compile-grammar.js
       reads grammar/enscribe.peggy → writes src/generated/parser.js
...
═══════════════════════════════════════════════════════════════════════════
  ✓ enscribe-core/*                                     (entire package)
  ✓ remark-enscribe/src/syntax.js, from-markdown.js,
    recursive-content.js, index.js, generated/parser.js
  ✓ enscribe-interpreter/src/lib/*                      (all interpreter-internal helpers)
  ✓ enscribe-interpreter/src/plugins/*                  (except library-load.js)
  ✓ enscribe-interpreter/src/handlers/*                 (except table.js's src= branch)
  ✓ enscribe-interpreter/src/schema/{shape-tokens,validate}.js
  ✓ enscribe-interpreter/src/interpret-plugin.js
  ✓ enscribe-interpreter/src/dsl/registry.js             (DSL asset-emit registry; no Node built-ins after the node-assets split)
  ✓ enscribe-interpreter/src/browser.js                  (the render/renderInto browser façade; Phase 14 Slice 1)
...
═══════════════════════════════════════════════════════════════════════════
  ✗ enscribe-interpreter/src/index.js's asset-injection paths (read fs)
  ✗ enscribe-interpreter/src/assets/font-loader.js
  ✗ enscribe-interpreter/src/handlers/table.js's <table src=…> branch
  ✗ enscribe-interpreter/src/plugins/library-load.js
  ✗ enscribe-interpreter/src/dsl/node-assets.js          (DSL live bundle loaders + jsdom abc→SVG static renderer; split from registry.js)
```
...
in `enscribe-interpreter/src/lib/` and were treated as an
interpreter-internal centralization. In Slice 3 they moved to
`enscribe-core/src/walkers/`, and the centralization scope broadens to span
all consumers: the interpreter today; the JATS export when it arrives; any
future output generator. Plugins and output generators that walk enscribe
...
(`enscribe-interpreter` used to import `dsl-registry` and the sigil
mapping — now `tagname-sigil-map` — from `remark-enscribe` directly). Three packages cannot — JATS-export would
have to pick which sibling to import from, and either choice picks an
arbitrary owner of facts that belong to none of them. `enscribe-core` is
the home for facts that belong to the project, not to a particular producer
or consumer.

**Drift prevention.** The `enscribeTag` shape, the error-node contracts,
the DSL registry — these are global-design facts. Restated facts drift. The
DRY audit found 12 hand-construction sites of the `enscribeTag` shape with
...
**Client-side build enablement.** Enscribe's eventual client-side build —
loading an `.emd` file directly in a browser, parsing and rendering without
...
runtime code to be free of `fs` / `path` / Node built-ins. `enscribe-core`
...
has no CDN to link to — enscribe's own `hover-preview.css/.js` — it is
...
| `2fabdf5` | 2 | Added the `enscribeTag` builder family in `src/tag.js`; migrated 12 hand-construction sites + the partial `lib/ast-helpers.js#makeTag` (DRY audit Bin A.1). |
