...
| `src/core/dsl-registry.js` | The language/type axis: the `LANGUAGES` map (identifier → handler + opacity), the derived flat `DSL_REGISTRY` (`tagname → contentHandler`), and the `getContentHandler` / `isOpaqueLanguage` API. (The per-language `(purpose, host)` bindings + `getLanguageBindings` were removed in #85 as inert data — host admission is owned by `interpreter/lib/host-accept-sets.js`.) | G3, G4 |
...
| `src/core/file-data-keys.js` | String constants for `file.data.*` plugin-bus keys (such as `ENSCRIBE_CONFIG`, `ENSCRIBE_DOC_TYPE`, `ENSCRIBE_STRICT_MODE`, `ENSCRIBE_REGISTRY`, `ENSCRIBE_CROSSREF_REGISTRY`, `ENSCRIBE_CITATIONS`, `ENSCRIBE_ASSETS`, `ENSCRIBE_LOADED_SOURCES`, `ENSCRIBE_NOTES_PENDING`, `ENSCRIBE_NUMBERING_PENDING`) | G12 |
