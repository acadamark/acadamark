...
The outer parse can leave a node's `content` as a **`(string | Node)[]` array** rather than a single string: string segments interleaved with already-resolved inline nodes the parse tokenized out of the body — the sigil-produced `<sup>`/`<sub>` of `^{}`/`_{}`, and the escape-error markers (`enscribeParseError`; `notes/specs/escape-rules-spec.md`) of failed escapes. The plugin parses **each string segment independently** through the inner pipeline, **splices the resolved nodes in place** (terminal — never re-tokenized), and assembles the fragments and nodes into one inline run per paragraph. Prose between markers parses correctly and errors do not cascade: one bad escape does not corrupt the parse of the surrounding text.

**Seam whitespace is content.** A string segment is parsed through markdown, which drops the segment's leading and trailing whitespace (a markdown paragraph carries no edge whitespace). That trim is correct at a paragraph or body **edge**, but a space where a segment abuts a resolved inline node is meaningful: `<p | x ^{2}>` is `x <sup>2</sup>`, not `x<sup>2</sup>`, and `<p | 1^{st} ed>` keeps the space after `</sup>`. The reassembly therefore restores one collapsed space at a string-segment ↔ inline-node **seam** (on either side), while a paragraph or body edge stays trimmed and a sigil glued to its neighbour (`1^{st}`, no space) stays glued. The rule keys on the seam, not on the sigil, so it holds uniformly for every resolved-node kind (sigils today; escape markers; any future pre-tokenized inline fragment).
...
A maximum recursion depth of 10 is enforced as a sanity check. When a node would be parsed beyond that depth, the plugin **converts the node in place to a sparse `enscribeParseError`**: it sets `subtype: "max-recursion-depth"` and `source: "<tagname>"`, clears every other `enscribeTag` field (keeping only `position`), and stops descending. This should never trigger in practice; it guards against infinite loops from malformed input or future bugs.
...
**Construction.** Fresh nodes use `makeParseError(subtype, source)` (`@enscribejs/enscribe/core/error-nodes.js`); the grammar (`enscribe.peggy`) emits the sparse shape directly. The recursion-limit case produces it by **in-place mutation** of the offending `enscribeTag` node — stamping `type` / `subtype` / `source` and clearing every other tag field, so it ends in the same sparse shape (with `position` preserved). Every `enscribeParseError`, whatever its construction site, is therefore uniformly sparse.
