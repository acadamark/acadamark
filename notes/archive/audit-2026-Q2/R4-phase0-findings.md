...
`enscribeLibraryLoad(options)` returns a `(tree, file)` closure that:

1. Finds all `<data>` nodes at `tree.children` level. These are placed at root level by `enscribeArticleStructuring` — `libraryLoad` depends on that having run first.
...
4. Reads `file?.data?.enscribeConfig?.get('citation-style')` for the style (default: `'chicago-author-date'`). **Requires `enscribeConfigDiscovery` to have run first.**

5. Writes one key: `file.data.enscribeCitations = { cite: mergedCite, order: [], style }`.
...
```js
// line 116
const citations = file?.data?.enscribeCitations;
...
**Other consumers:** None. `grep -r 'enscribeCitations' src/` returns only `library-load.js`, `cite-resolution.js`, and `bibliography.js` (plus comments in `index.js`).

**Dependency on plugin vs. direct-call:** Both consumers read from `file.data.enscribeCitations`. They do not care whether the key was set by a unified plugin call or a direct function call. The only requirement is that the key be present before step 10 runs.
...
- Single output: `file.data.enscribeCitations`.
...
1. **`enscribeArticleStructuring` must have run first** — `libraryLoad` finds `<data>` at `tree.children`; article-structuring is what moves `<data>` there. If `libraryLoad` runs before article-structuring, `dataNodes` is always empty and `file.data.enscribeCitations` is never set.

2. **`enscribeConfigDiscovery` must have run first** — `libraryLoad` reads `file.data.enscribeConfig` for the citation style. If config-discovery has not run, the style falls back to the default ('chicago-author-date'), which is correct behavior but means config-controlled citation styles won't take effect.
...
2. enscribeConfigDiscovery   ← before article-structuring
3. enscribeArticleStructuring
4. enscribeSectionNesting
5. enscribeLibraryLoad       ← after article-structuring
```
...
**`src/index.js`:** The `this.use(enscribeLibraryLoad, { assetsDir })` line becomes part of an explicit "index inputs" block. Concretely: either a comment reclassification, or wrapping the call inside a combined plugin:

```js
// ── Index inputs: build config + citation indexes. ──────────────────────────
this.use(function indexInputs() {
  return (tree, file) => {
    enscribeConfigDiscovery()(tree, file);   // keeps config-discovery in place
...
**`cite-resolution.js`, `bibliography.js`:** No change. They read `file.data.enscribeCitations`; the key is unchanged.

**`test/plugins/library-load.test.js`:** No change. All 7 tests invoke `enscribeLibraryLoad({ assetsDir })(tree, file)` — the plugin-calling convention. If the exported function is renamed or wrapped, the test import is updated, but the test logic is unchanged.
...
- All three failure paths leave `file.data.enscribeCitations` unset, which both `cite-resolution` and `bibliography` handle as a no-op (`if (!citations) return`).
...
`test/integration.test.js` — the manual pipeline mirror (line 67) calls `enscribeLibraryLoad({ assetsDir })(mdast, file)` directly. If Shape A keeps the same export name and signature, this test requires no update. If the function is renamed, the import on line 32 needs updating.
...
| F2 | Ordering dependency | `libraryLoad` depends on `enscribeArticleStructuring` having run (to find `<data>` at root) and on `enscribeConfigDiscovery` having run (to read `citation-style`). The full `indexInputs` consolidation cannot happen without verifying that moving config-discovery to after article-structuring is safe. |
...
2. **`index.js`**: The `this.use(enscribeLibraryLoad, { assetsDir })` call becomes an explicit index-build call inside a clearly labeled block. Config-discovery stays where it is (step 2, before article-structuring) — its reordering is a follow-on, not required for R4.
