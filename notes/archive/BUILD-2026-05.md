...
> **Planning-era document.** This document was written before the interpreter was implemented. The pipeline diagram and plugin descriptions below reflect the planned architecture, not the current implementation. Plugin names (`enscribeTagInterpret`, `enscribeCitations`, `enscribeCrossRefs`, `rehypeKatex`, `rehypeShiki`) and the rehype-plugin architecture described here were superseded. For the current pipeline architecture and actual plugin names, see:
...
This document describes how enscribe is implemented. It assumes you've read `DESIGN.md`.
...
The next focus is the **first interpreter slice**: build `enscribeTagInterpret` for a small set of in-scope structural elements (article, section, sub-section, sub-sub-section, p, aside, blockquote, hr, figure, ul, ol, li, em, strong, code, meta) plus the structural plugins that produce `<article-front>`/`<article-body>`/`<article-back>`, section-titles, and section nesting. After this slice, simple structural enscribe documents render to real HTML end-to-end. The architecture and slice scope are described in `notes/interpreter-design.md` and `notes/plugin-pipeline.md`.
...
Enscribe is built as a set of plugins on the [unified](https://unifiedjs.com/) ecosystem. This decision was made after considering three options:
...
2. **Two tree dialects.** *mdast* is the markdown AST. *hast* is the HTML AST. Markdown parses to mdast; mdast converts to hast; hast serializes to HTML. Enscribe plugins live on one or both trees.

3. **Named pieces.** *remark* handles markdown↔mdast. *rehype* handles HTML↔hast. *unified* chains them. *micromark* is the lower-level tokenizer used only when inventing genuinely new syntax (i.e., the enscribe shorthand).
...
unified()
  ...
  .use(enscribeShorthand)             // NOVEL: parse <tag | content>
  .use(remarkMath)                     // borrowed: $...$ and $$...$$
  .use(remarkGfm)                      // borrowed: tables, footnotes
  .use(remarkRehype, { allowDangerousHtml: true })  // mdast → hast
  .use(enscribeTagInterpret)          // NOVEL: shorthand nodes → real HTML
  .use(enscribeSectionNesting)        // NOVEL: implicit-close sections
  .use(enscribeNumbering)             // NOVEL: figure/section/eq numbers
  .use(enscribeCitations)             // NOVEL: resolve <cite> + bib
  .use(enscribeCrossRefs)             // NOVEL: resolve <ref>
...
The pipeline above produces HTML — enscribe's primary output. A *parallel* pipeline produces JATS XML by replacing `rehypeStringify` with a JATS converter and inserting `rehypeEnscribeToJats` before serialization. Both pipelines start from the same Layer 1 hast tree, diverging only at the final stages. See "What's novel" below for `rehypeEnscribeToJats`.

Lines marked NOVEL are enscribe's contribution. Everything else is borrowed.
...
### 1. `enscribeShorthand` — the shorthand parser
...
This is the only place micromark is touched. A grammar in roughly 100–200 lines covers the syntax. Output is mdast nodes of a generic type `enscribeTag` with parsed `name`, `id`, `classes`, `attributes`, `flags`, and `content` properties. The parser does *not* know what individual tags mean — it produces uniform generic nodes.

### 2. `enscribeTagInterpret` — semantic interpretation

A separate plugin that walks the tree and transforms `enscribeTag` nodes into proper HTML based on the tag name. This is where each tag's Layer 1 semantics live: `<figure>` becomes a `<figure>` with nested `<img>` and `<figcaption>`; `<cite>` becomes a `<cite>` with the right `data-*` attributes; etc.
...
### 3. `enscribeSectionNesting` — implicit-close sections
...
### 4. `enscribeNumbering` — figure/section/equation numbers
...
### 5. `enscribeCitations` — citation resolution
...
### 6. `enscribeCrossRefs` — cross-reference resolution
...
### 7. `rehypeEnscribeToJats` — JATS XML export

A converter plugin (technically not a rehype plugin in the strict sense, since the output is XML rather than HTML, but conceptually parallel). Takes a Layer 1 hast tree and produces JATS XML. Maps enscribe custom elements (`<article-title>`, `<section-title>`, `<sub-section>`, etc.) to their JATS equivalents (`<article-title>`, `<title>` inside `<sec>`, nested `<sec>` elements). Restructures where JATS nesting differs from enscribe's. Pads out required JATS metadata with sensible defaults or explicit author-provided values from a frontmatter block.

The enscribe-to-JATS mapping table is the heart of this plugin. Most mappings are 1:1 element renames. A minority require restructuring — for example, enscribe's flat-then-nested section model maps cleanly onto JATS's recursive `<sec>` model, but enscribe's `<article-title>` plus `<article-subtitle>` becomes JATS's `<title-group>` containing `<article-title>` and `<subtitle>`.

This plugin is what makes enscribe a credible scholarly-publishing target rather than just "another web markdown."

## Design principle: parser knows nothing about meaning

The shorthand parser produces generic `enscribeTag` nodes. A separate interpretation pass converts those nodes into specific HTML based on tag name. This separation is enforced because:
...
5. Write the `enscribeTagInterpret` plugin with rules only for the stable-subset tags.
...
10. Add interpretation rules for `<cite>`, `<ref>`, `<figure>`, and the bibliography to `enscribeTagInterpret`.
...
16. Implement `rehypeEnscribeToJats` and demonstrate round-trip: an enscribe document → Layer 1 HTML → JATS XML → validated against the JATS schema.

## Shorthand parser — slice map

The shorthand parser (`packages/remark-enscribe`) is built in vertical slices. Each slice adds a self-contained capability with passing tests before the next begins. A drift check against `notes/shorthand-syntax.md` is performed at the end of each slice.
