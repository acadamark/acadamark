...
- [ ] Multi-file source — a master document (`<meta>` / structure / `<data>` / `<config>`) that assembles `<section src>` child files into one article (design of record: `notes/specs/master-document.md`; epic [#190](https://github.com/enscribejs/enscribe/issues/190)). A **walking skeleton** exists and is tested (`enscribe build`: parse the master → load + parse each child → assemble in document order → render as one article); cross-file citations/bibliography, numbering and cross-references, placement markers (toc / endnotes / bibliography), and non-article types (book, website) are pending
...
- [x] `enscribe build` — assemble a multi-file master document, then render it (walking skeleton; [#190](https://github.com/enscribejs/enscribe/issues/190) — see the multi-file source row above for what is and isn't assembled yet)
