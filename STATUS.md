...
- [ ] Multi-file source — remaining pieces: placement markers (`<toc>` / `<endnotes>` / `<bibliography>` display at authored positions, incl. per-chapter `split_bib`) and the **website** document type. *(The book type shipped — see the book-level row above; master-level `<library src>` live-loading shipped in [#197](https://github.com/enscribejs/enscribe/issues/197); the **cross-file citation registry merge** shipped — every `<library>` across the master and its children merges into one project-wide registry, so a `<cite>` in any file resolves against a reference declared anywhere, identically across the static single-page build, the default separate-pages book build, and the live render ([#190](https://github.com/enscribejs/enscribe/issues/190)).)*
