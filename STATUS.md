...
- [x] Live app-shell book render (editing-surface track, L2, read-only) — a single app-shell `index.html` renders a book **live in the browser from its `.emd` source**: it fetches the master + its chapter children, runs the cheap global pass once (numbering + cross-reference resolution), then renders the **current** chapter via the same per-chapter projection the static build publishes, mounted with the reading-interface chrome and routed by URL hash (`index.html#1-counting-elephants`, the same per-chapter slug scheme), rendering each chapter lazily on navigate and caching the rendered ones. Cross-chapter references resolve live with **no per-file href rewrite** — under one mount a bare `#anchor` routes to its owning chapter via the harvested registry, then scrolls. The live per-chapter content is byte-identical to the static page's content (the published≡live parity, proven). Browser entry `mountLiveBook` / `mountLiveBookFromUrl`; served over HTTP (the shell `fetch`es). Read-only this slice — the **edit loop** (re-render only the edited chapter on change) is next, and `enscribe serve` is a later convenience (the shell is a dev-preview fixture for now)
