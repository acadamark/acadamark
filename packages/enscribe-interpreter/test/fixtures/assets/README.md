# Enscribe example document assets

This folder contains image assets for use in enscribe example documents
(`document-5-linear-regression.emd`, `document-6-cross-references.emd`).
...
Use in `document-5-linear-regression.emd`:
...
For `document-6-cross-references.emd`, the figure example references an
...
question if enscribe itself is later relicensed.
