 ...
 *
 * Walks the mdast tree and re-parses string content for any `acadamarkTag`
 ...
 *   const inner = unified().use(remarkParse).use(remarkAcadamark) // ... etc.
 *   unified()
 *     .use(remarkParse)
 *     .use(remarkAcadamark)
 ...
 * `(string | acadamarkParseError)[]` value, each string segment is parsed
 ...
 * `acadamarkParseError` with subtype `max-recursion-depth`. In practice,
...
function processNodes(subtree, processor, depth) {
  visit(subtree, 'acadamarkTag', (node) => {
    ...
    // acadamarkTag nodes that themselves have string content.
    processNodes(
...
function parseContent(content, processor) {
  ...
  // content is (string | acadamarkParseError)[] from escape processing.
