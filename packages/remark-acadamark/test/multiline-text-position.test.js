...

{
  ...
  assert.ok(tag, 'inline acadamarkTag found inside paragraph')
...

{
  ...
  const para = tree.children.find((n) => n.type === 'paragraph')
  assert.ok(para, 'paragraph exists')
  const tagIdx = para.children.findIndex((n) => n.type === 'acadamarkTag')
...

{
  ...
  const para = tree.children.find((n) => n.type === 'paragraph')
  const tag = para?.children?.find((n) => n.type === 'acadamarkTag')
  assert.ok(tag, 'inline acadamarkTag found')
  assert.equal(tag.content, ' line one\nline two\nline three.')
...

{
  ...
  assert.ok(para, 'paragraph exists')
  const tag = para.children.find((n) => n.type === 'acadamarkTag')
  assert.ok(tag, 'inline acadamarkTag found (not error)')
  assert.equal(tag.type, 'acadamarkTag')
  assert.equal(tag.tagname, '```')
...
{
  // No acadamarkTagError nodes produced for any of the above
  const cases = [
    'A <note | line one\nline two.> B.',
    'A <` foo\nbar `> B.',
    'A <``` code\nblock ```> B.',
  ...
  for (const src of cases) {
    ...
    const errNodes = tree.children.flatMap((n) =>
      n.type === 'acadamarkTagError' ? [n] :
      (n.children || []).filter((c) => c.type === 'acadamarkTagError')
    )
    assert.equal(errNodes.length, 0, `no error nodes for: ${JSON.stringify(src)}`)
  }
  console.log('PASS: no acadamarkTagError nodes for multi-line inline tags')
}
