 ...
 * a FLOW (block-level) acadamarkTag. When the same sigil or named tag appears
...

{
  ...
  assert.equal(node.type, 'acadamarkTag', 'top-level node is acadamarkTag (flow)')
...

{
  ...
  assert.ok(tag, 'inline acadamarkTag inside paragraph')
...

{
  ...
  assert.equal(tags.length, 2, 'two inline acadamarkTags')
...

{
  ...
  const tree = parse(src)
  assert.equal(tree.children.length, 1, 'one top-level node')
  assert.equal(tree.children[0].type, 'acadamarkTag', 'flow acadamarkTag')
...

{
  ...
  const tree = parse(src)
  const flowTag = tree.children.find((n) => n.type === 'acadamarkTag')
  assert.ok(flowTag, 'flow acadamarkTag present')
