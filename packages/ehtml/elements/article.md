...
`<article>` is a real eHTML element that appears in output, but it's typically *generated by the structural plugin* rather than authored as a wrapper. The typical authoring path is:
...
In semantic mode (the default), `<article>` and its children remain as eHTML elements with explicit semantics. Render-mode lowering of the article structure to plain HTML (`<article-front>` → `<header>`, `<article-title>` → `<h1>`, etc.) is a future, unbuilt feature ([#40](https://github.com/enscribejs/enscribe/issues/40)) specified in [`notes/specs/render-mode.md`](../../../../notes/specs/render-mode.md).
