# eHTML Vocabulary Specification

The enscribe eHTML semantic HTML vocabulary. This is the target for enscribe's interpreter, JATS export, and (optionally) hand-authoring.
...
The authoring shorthand `<chapter>` expands at the parser layer to `<book-part type="chapter">`; it is not a separate eHTML element. Other shorthand expansions follow the same pattern (`<part>`, `<appendix>`, `<preface>`, etc.).
...
In practice, authors rarely write the container elements directly. The typical authoring path is `<meta type=...>` plus content; the structural plugin reads the `type` kwarg and generates the container plus region wrappers. The container elements are real eHTML elements that exist in output for semantic structure and clean JATS export. They remain available as explicit-form escape hatches.
...
`<meta>` is the metadata container that authors place at the top of a document. Its `type` kwarg (`article`, `book`, `book-part`) tells the structural plugin which eHTML wrapper to generate. `<meta>` survives in the output, placed inside `<article-front>` (or `<book-front>`, or directly inside `<book-part>`) by the structural plugin.
...
Captioned, numbered, self-contained content uses HTML-native `<figure>` with `<figcaption>` for captions. Tables, images, code listings, equations, and diagrams that need a caption are all wrapped in `<figure>`. Per Rule 2, no eHTML custom element is introduced — both `<figure>` and `<figcaption>` are standard HTML.
...
**Lists (#137).** `<list>` / `<list ordered>` is the canonical authoring container (explicit `</list>` close); `<ul>` / `<ol>` / `<li>` are its HTML **render output, not authoring vocabulary**. The `ordered` positional selects an ordered list (`<ol>`); the default is unordered (`<ul>`). Items are written with **bare markers** — `<li>` (canonical), the `<->` / `<*>` sigils, or the `-` / `*` markdown idiom — each item's content being the flow that follows its marker, peer-closed by the next marker, a nested `<list>`, or `</list>` (no pipe, no per-item wrapper). Markers are recognized only at flow position, never inline, so prose `<-` / `->` is safe. Multi-paragraph items and nested `<list>`s are supported; `marker=` (CSS `list-style-type`), `start=N`, and `reversed` are honored on ordered lists. A `<list>` lowers to a markdown list node and reuses the existing list render; on export it becomes JATS `<list list-type="bullet|order">` with `<list-item>`. See `notes/specs/lists.md`.
...
| `<note>` | `<fn>` (position `end`/`foot`) or `<boxed-text content-type="marginnote">` (position `margin`) | Footnote / endnote / margin note. One note type, three positions (foot, end, margin); position drives the JATS element, numbering is independent (#333). A `<note position=margin>` is projected into the page margin beside its marker. |

**The vocabulary keys by authoring name, decoupled from the HTML element it renders to.** Each vocabulary entry is keyed by its authoring tagname (its filename stem under `elements/`), not by the HTML element it lowers to — so an element whose authoring name differs from the HTML element it renders can coexist with a distinct entry for that HTML element, no collision. This is the contract the data generator (`build/generate-data-module.js`) enforces.
...
These don't need new eHTML elements. Display math can be wrapped in `<figure>` for numbering and captioning.
...
- **Embedded DSL elements.** Mermaid, ABC notation, etc. Handled by long-form DSL tags at the parser layer; their eHTML representation is `<figure>` containing appropriate child content.
...
1. **Distinct container elements (Option Y).** `<article>`, `<book>`, and `<book-part>` are separate elements rather than a single recursive `<article>` with `document-type` distinctions. This matches BITS, makes JATS export simpler, and gives authors a more discoverable vocabulary. The cost is a slightly larger element list; the cost is bounded. Chapters, parts, appendices, etc. are `<book-part>` instances disambiguated by the `type` kwarg, not separate eHTML elements; the parser exposes them as authoring shorthands (`<chapter>`, `<part>`, `<appendix>`).
...
8. **Section depth as named ladder, not recursive nesting.** `<section>` / `<sub-section>` / `<sub-sub-section>` is the depth ladder. Per `notes/specs/ehtml-naming.md` Rule 3.

9. **Document type via `<meta type=...>`.** Authors declare document type as a kwarg on `<meta>` (`type=article` / `type=book` / `type=book-part`) rather than wrapping content in `<article>` / `<book>` / `<book-part>`. The structural plugin reads the type kwarg and generates the appropriate eHTML wrapper structure: article + front/body/back; book + front/body/back; book-part containing `<meta>` and body content directly (no nested front/body/back wrappers). The container elements (`<article>`, `<book>`, `<book-part>`) are real eHTML elements that exist in output for semantic structure and clean JATS export. Authors can still write them explicitly as an escape hatch. The `<book-part-meta>` element was removed (May 2026) because book-parts use the same `<meta>` container as articles and books.

10. **Heading level is structural, never authored (#397).** A title element's outline level derives from container nesting — the depth ladder (convention 8) plus any enclosing `<book-part>`s — so the archival vocabulary carries no `<h1>`–`<h6>` and no level attribute: level is a computed fact, not authored content (the no-computable-facts rule applied to the format itself; HTML's own deleted outline algorithm is the cautionary precedent for authored-level drift). Rendered projections MATERIALIZE the semantics: the renderer wraps each outline title's inline content in the native heading of its derived level (`<section-title><h2>…</h2></section-title>` — eHTML outside, native heading inside), the same derived-display category as materialized section numbers and resolved citations. The wrap is the conforming default (`heading-tags`, default on); level derivation, the outline set, and the overflow rule are specified in `notes/specs/render-quality.md` RQ-DOC-M4.
...
produces and operates on eHTML elements is in `notes/specs/pipeline.md`.
