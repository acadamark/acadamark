...
export function run() {
  ...
  {
    assert.ok(mermaidSkip.includes('data-acadamark-dsl="mermaid"'), 'skip: mermaid contract markup present');
    ...
    assert.ok(abcSkip.includes('data-acadamark-dsl="abc"'), 'skip: abc contract markup present');
  ...
  // ── live-inline mermaid: bundle inlined + init, contract preserved ────────────
  {
    ...
    assert.ok(html.includes('data-acadamark-dsl="mermaid"'), 'live-inline: contract markup preserved');
  ...
  // ── live-inline / live-link abc ───────────────────────────────────────────────
  {
    ...
    assert.ok(/<pre[^>]*data-acadamark-dsl="abc"/.test(inline), 'live-inline: abc <pre> contract preserved');
  ...
  // ── per-DSL override at the pipeline level ────────────────────────────────────
  {
    ...
    assert.ok(html.includes('data-acadamark-dsl="mermaid"'), 'override: contract markup still present');
