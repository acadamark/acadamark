...
import { acadamarkTagHandler } from '../src/interpret-plugin.js';
...
export function run() {
  ...
  // --- unknown tag → span with data-acadamark-unknown ---
  {
    const node = makeAcadaTag('not-a-real-tag', [para('content')]);
    const h = hast(node);
    assert.equal(h.tagName, 'span');
    assert.equal(h.properties['data-acadamark-unknown'], 'not-a-real-tag');
