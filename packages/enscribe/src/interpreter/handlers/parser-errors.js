...
//
//   enscribeTagError — produced when a tag opener parses but the closer is
//     missing (sigil opener that reaches EOF; long-form opener that reaches
//     EOF). Two variants:
//       Sigil-opener: { type, source, error, position? }
//       Long-form:    full enscribeTag fields + `error` (no `source`)
//
...
/**
 * Handler for enscribeParseError mdast nodes.
 *
 * @param {object} _state - mdast-util-to-hast state (unused — leaf node)
 * @param {object} node   - enscribeParseError node
...
/**
 * Handler for enscribeTagError mdast nodes. Branches on the two variants per
 * the error-node spec: sigil-opener (sparse, has `source`) vs long-form
 * (full tag fields + `error`).
 *
 * @param {object} _state - mdast-util-to-hast state (unused — leaf node)
 * @param {object} node   - enscribeTagError node (either variant)
 * @returns {import('hast').Element}
 */
