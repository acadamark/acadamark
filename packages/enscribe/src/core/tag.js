...
//     table) as canonical enscribeTag nodes. The caller must supply
...
//     `__note-list-item`, etc. into the tree. These are `enscribeTag` nodes
...
/**
 * General-purpose short-form enscribeTag builder. Parsed content (array of
 * child nodes); default contentHandler.
 *
 * @param {string} tagname
 * @param {Array} [content] - child nodes (mdast / enscribeTag); default []
 * @param {object} [opts]
 ...
 * @returns {object} enscribeTag node
 */
...
/**
 * Opaque-content short-form enscribeTag builder. Used when the content is a
...
/**
 * Internal-marker short-form enscribeTag builder. Used for interpreter-
 ...
 */
/**
 * Predicate: returns true iff `node` is an enscribeTag node. With `name`
 ...
 * @returns {boolean}
 */
export function isEnscribeTag(node, name) {
  if (!node || node.type !== 'enscribeTag') return false;
