// Tests for the enscribeNotePlacement plugin — the four required tests from
// R3a Phase 0 §6.2.
//
// Tests:
...
//   enscribeNotes → enscribeNumbering → numberRegistry → fillNumbering
//   → enscribeRefResolution (or enscribeCiteResolution) → enscribeNotePlacement
//
...
import { enscribeNotes } from '../../src/interpreter/plugins/notes.js';
import { enscribeNotePlacement } from '../../src/interpreter/plugins/note-placement.js';
...
function findAllTags(nodes, tagname) {
  ...
  for (const n of (nodes ?? [])) {
    if (isEnscribeTag(n, tagname)) found.push(n);
...
function runPipeline(tree, file) {
  ...
  enscribeNotePlacement()(tree, file);
}
...
@article{jones2024,
  ...
  journal = {Other Journal}
    ...
    const file = { data: {}, message: () => {} };
    file.data.enscribeCitations = { cite: citeInstance, order: [], style: 'apa' };
