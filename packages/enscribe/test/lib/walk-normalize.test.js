...
import { walkNormalize } from '../../src/core/walkers/walk-normalize.js';
...
function normalizeMath(node) {
  return { type: 'enscribeTag', tagname: '$', content: node.value, isOpaqueContent: true };
}
...
export function run() {
  ...
  // --- predicate matching: matching node is replaced ---
  {
    ...
    assert.equal(nodes[0].type, 'enscribeTag');
  ...
  // --- mixed array: only matching nodes replaced ---
  {
    ...
    assert.equal(nodes[0].type, 'text');
    assert.equal(nodes[1].type, 'enscribeTag');
  ...
  // --- descent into mdast .children ---
  {
    ...
    assert.equal(para.children[1].type, 'enscribeTag');
  ...
  // --- descent into non-opaque enscribeTag .content ---
  {
    ...
    assert.equal(aside.content[1].type, 'enscribeTag');
    assert.equal(aside.content[1].content, 'w');
    console.log('PASS: walkNormalize: descends into non-opaque enscribeTag .content');
  }

  // --- does NOT descend into opaque enscribeTag .content ---
  {
    ...
    console.log('PASS: walkNormalize: does NOT descend into opaque enscribeTag .content');
  }
  ...
  // --- deeply nested: paragraph inside non-opaque tag content ---
  {
    ...
    const nodes = [aside];
    walkNormalize(nodes, isPureMath, normalizeMath);
    assert.equal(para.children[0].type, 'enscribeTag');
  ...
  // --- replacement node is descended into (non-opaque replacement) ---
  {
    ...
    function normalizeWrapper(node) {
      // Returns a non-opaque enscribeTag whose content is the wrapper's children.
      return {
        type: 'enscribeTag',
        tagname: 'section',
    ...
    assert.equal(section.type, 'enscribeTag');
    assert.equal(section.tagname, 'section');
    // The inner inlineMath inside content should also have been normalized.
    assert.equal(section.content[0].type, 'enscribeTag', 'inner math normalized after replacement descent');
