...
// tree, calling registered visitor callbacks for each matched acadamarkTag node.
//
// visitors — Map<string, (node: object) => void>
//   Keyed by tagname. When a matching acadamarkTag is encountered, its visitor
 ...
 * For each acadamarkTag node whose tagname is in `visitors`, call the visitor.
 *
 * @param {Array} nodes
...
function walkDiscover(nodes, visitors) {
  ...
  for (const node of nodes) {
    if (isAcadamarkTag(node)) {
      ...
      // Recurse into acadamarkTag content. Skip opaque content — math bodies,
 ...
 * callbacks for each matching acadamarkTag node.
 *
