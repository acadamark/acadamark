...
//
// The lift recorded as deferred in `notes/specs/acadamark-core.md`
// §"Output-stage-specific code" (the only architectural item the
// acadamark-core extraction arc left open, per commit `6ae6844`). The
// deferral was waiting for a second output-target consumer — JATS export
// (Phase 5) is that second consumer.
//
// THE PROBLEM
//
// Both HTML rendering (acadamark-interpreter) and JATS export
// (acadamark-jats-export) need to walk a node's id / classes / kwargs
...
// `acadamark-interpreter/src/lib/build-properties.js` as `buildProperties`
...
export function mapAttributes(node, vocab, target, emit) {
  ...
  // id
  if (node.id) {
    const def = vocab?.acadamark_attributes?.id;
  ...
  if (Array.isArray(node.classes) && node.classes.length > 0) {
    const def = vocab?.acadamark_attributes?.classes;
    const name = def?.maps_to?.[target];
    if (name) {
