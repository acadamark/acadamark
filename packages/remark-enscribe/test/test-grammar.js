 ...
 * single enscribe construct (as the micromark finder would extract it).
 * Output is an enscribeTag node object.
 *
...
{
  // Markdown pass-through: \* is not an enscribe escape — passed unchanged
  const n = p('<aside | a \\* b>')
  assert.equal(typeof n.content, 'string')
  assert.equal(n.content, ' a \\* b')
...
{
  // Unknown escape \q → enscribeParseError node in content array
  ...
  assert.equal(n.content[1].type, 'enscribeParseError')
  assert.equal(n.content[1].subtype, 'unknown-escape-sequence')
  assert.equal(n.content[1].source, '\\q')
  assert.equal(n.content[2], ' more')
  console.log('PASS grammar: \\q in named-tag content → enscribeParseError node')
}

{
  ...
  console.log('PASS grammar: trailing \\ in named-tag content → enscribeParseError node')
}
...

{
  ...
  console.log('PASS grammar: \\q in hash sigil body → enscribeParseError node')
}
...
// path backslashes produced enscribeParseError nodes that split the content
...

{
  ...
  const supNode = items.find((it) => it?.type === 'enscribeTag' && it.tagname === 'sup')
...

{
  ...
  const subNode = n.content.find((it) => it?.type === 'enscribeTag' && it.tagname === 'sub')
...

{
  ...
  assert.ok(Array.isArray(n.content), 'content is array')
  const supNode = n.content.find((it) => it?.type === 'enscribeTag' && it.tagname === 'sup')
  ...
  const subNode = supContent.find((it) => it?.type === 'enscribeTag' && it.tagname === 'sub')
...
{
  // Nested enscribe construct inside braces: ^{see <cite @jones>}.
  ...
  console.log('PASS grammar G1a-6: nested enscribe construct in brace content (raw text at grammar level)')
}
...
{
  // Error: empty braces ^{} → enscribeParseError.
  const n = p('<aside | ^{}>')
  assert.ok(Array.isArray(n.content), 'content is array')
  const errNode = n.content.find((it) => it?.type === 'enscribeParseError')
  assert.ok(errNode, 'enscribeParseError present for empty ^{}')
  assert.equal(errNode.subtype, 'empty-shortcut')
  assert.equal(errNode.source, '^{}')
  console.log('PASS grammar G1a-9: ^{} → enscribeParseError (empty-shortcut)')
}

{
  // Error: unterminated ^{abc → enscribeParseError.
  const n = p('<aside | ^{abc>')
  assert.ok(Array.isArray(n.content), 'content is array (has error)')
  const errNode = n.content.find((it) => it?.type === 'enscribeParseError')
  assert.ok(errNode, 'enscribeParseError present for unterminated ^{abc')
  assert.equal(errNode.subtype, 'unterminated-shortcut')
  console.log('PASS grammar G1a-10: ^{abc (unterminated) → enscribeParseError')
}
