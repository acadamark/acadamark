...
content:
  type: opaque
  notes: |
    Author writes Mermaid diagram source verbatim. Enscribe preserves
...
handler_responsibilities:
  ...
  - Emit a `<pre class="mermaid" data-enscribe-dsl="mermaid">…</pre>`
...
A Mermaid diagram block. External DSL — the source is preserved as marked markup; rendering to SVG happens external to enscribe.

## Semantic intent

`<mermaid>` is enscribe's tag for Mermaid diagram source (flowcharts, sequence diagrams, Gantt charts, etc.). The handler emits a `<pre class="mermaid" data-enscribe-dsl="mermaid">…</pre>` wrapper that preserves the diagram source. Rendering happens external to enscribe by one of two paths:

1. **View-time rendering** — the consumer's HTML includes Mermaid's library from a CDN (e.g. `<script src="https://cdn.jsdelivr.net/npm/mermaid@10/dist/mermaid.min.js"></script>`). Mermaid's default initialization scans the DOM for `class="mermaid"` elements and replaces their content with rendered SVG. The enscribe wrapper matches that convention, so no extra wiring is needed.

2. **Build-time rendering** — a pre-render pass (running Mermaid in a headless browser context) finds blocks by their `data-enscribe-dsl="mermaid"` attribute and replaces the source with rendered SVG before publication. The `data-enscribe-dsl` attribute is the enscribe-specific contract; it's independent of Mermaid's CDN class convention, which may change between versions.
...
The content is Mermaid source, preserved verbatim. Any Mermaid diagram type (flowchart, sequence, class, state, ER, Gantt, etc.) works — enscribe doesn't parse the source.
