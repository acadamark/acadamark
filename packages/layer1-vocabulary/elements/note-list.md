...
This element is normally **not authored directly**. The `enscribeNotePlacement` plugin generates `<note-list>` elements based on the document-level `note-position` setting, collecting notes from throughout the document into the appropriate position.
