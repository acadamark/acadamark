...
| `packages/ehtml/elements/*.md` + the generation rules in `notes/specs/documentation.md` | `docs-source/{enscribe_vocabulary,ehtml,authoring_guide}/*.emd` (the generated doc pages; #223) | `npm run docs:gen` (repo root) | `docs-gen/check-docs-fresh.mjs` — the root `docs:check` script + the CI `test.yml` step (#373) |
| the live-shell emitter (`packages/enscribe/src/shell/emit-shell.js`) + each example's `.emd` source(s) | `examples/{regression-live,savanna-live}/index.html` (committed emitter output; the four copied sibling assets are gitignored droppings, never committed) | the CLI build named in each dir's `.gitignore` header, e.g. `node packages/cli/bin/enscribe.js build examples/regression-live/article.emd --live -o examples/regression-live` | `scripts/check-examples-fresh.mjs` — the root `examples:check` script + the CI `test.yml` step (#390) |
| `packages/enscribe/src/interpreter/assets/default.css` + `assets/themes/*.css` (the shipped `--enscribe-*` token values) | the token-contract **table region** in `notes/specs/themes.md` (each token's name, default, and which themes retune it; #398) | `node scripts/gen-theme-tokens.mjs` (repo root) | `scripts/gen-theme-tokens.mjs --check` — the root `check:theme-tokens` script + the CI `test.yml` step |
| `packages/enscribe/src/interpreter/assets/default.css` + `assets/themes/*.css` (the LIGHT tokens) + the per-role rules in `scripts/lib/theme-token-roles.mjs` | the derived **dark blocks** baked into those same stylesheets (`@media (prefers-color-scheme: dark)` + the `data-theme-variant` hook; #398) | `node scripts/gen-dark-variants.mjs` (repo root) | `scripts/gen-dark-variants.mjs --check` (WCAG-AA contrast gate) — the root `check:dark` script + the CI `test.yml` step |
